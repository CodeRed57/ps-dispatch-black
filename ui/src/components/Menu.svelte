...
<div class="w-screen h-screen flex items-center justify-end { menuRight ? 'flex-row' : 'flex-row-reverse' } " transition:fly="{{ x: menuRight ? 400 : -400 }}">
  ...
  <div class="w-[3.2vh] h-[85%] flex flex-col gap-[1vh] " class:ml-[1vh]={!menuRight} class:mr-[1vh]={menuRight}>
    ...
    <!-- REFRESH ALERTS -->
    <button class="w-full h-[3vh] flex items-center justify-center rounded-lg bg-primary hover:bg-tertiary"
  ...
  <div class="w-[25%] h-[97%] overflow-auto pr-[0.5vh]" class:ml-[2vh]={!menuRight} class:mr-[2vh]={menuRight}>
    ...
    <button class="w-full h-fit mb-[1vh] font-medium rounded-xl {dispatch.priority == 1 ? 'bg-priority_secondary' : 'bg-secondary'}" on:click={() => toggleDispatch(dispatch.id)}>
        <div class="flex items-center gap-[1vh] p-[1vh] rounded-xl text-[1.5vh] {dispatch.priority == 1 ? " bg-priority_primary" : " bg-primary"}">
            <p class="px-[2vh] py-[0.2vh] rounded-xl bg-tertiary">
              #{dispatch.id}
            </p>
            <p class="px-[2vh] py-[0.2vh] rounded-xl {dispatch.priority == 1 ? " bg-accent_red" : "bg-tertiary"}">
              {dispatch.code}
            </p>
            <p class="py-[0.2vh]">
              {dispatch.message}
            </p>
            <i class="{dispatch.icon} py-[0.2vh] ml-auto mr-[0.5vh] rounded-xl {dispatch.priority == 1 ? " text-accent_red" : "text-tertiary"}"></i>
          </div>
        ...
        <div class=" mb-[1vh]" transition:slide={{ duration: 300 }}>
          {#if dispatch.units.length > 0}
            <div class="flex flex-col gap-[0.2vh] rounded-xl mb-[1vh] bg-primary">
              {#each dispatch.units.slice(0, additionalUnitsVisible[dispatch.id] ? dispatch.units.length : 3) as unit}
                <div class="w-full h-[5vh] rounded-xl flex {dispatch.priority == 1 ? 'bg-accent_red' : 'bg-secondary'} flex items-center font-medium">
                  <p class="ml-[2vh] px-[1.4vh] py-[0.2vh] rounded-xl {dispatch.priority == 1 ? 'bg-secondary' : 'bg-secondary'}">{unit.metadata.callsign}</p>
                  <p class="mx-[1vh] px-[1.5vh] py-[0.2vh] rounded-xl uppercase {unit.job.type == "leo" ? "bg-tertiary " : unit.job.type == "ems" ? "bg-tertiary" : "bg-tertiary" }">{unit.job.name}</p>
              ...
              {#if dispatch.units.length > 3}
                {#if !additionalUnitsVisible[dispatch.id]}
                  <button class="w-full h-[5vh] flex items-center rounded-xl justify-center {dispatch.priority == 1 ? 'bg-priority_tertiary' : 'bg-tertiary'} flex items-center font-medium" on:click={() => toggleAdditionalUnits(dispatch.id)}>
          ...
          {/if}
          <button class="w-full h-[5vh] rounded-xl {dispatch.priority == 1 ? " bg-priority_quaternary" : " bg-secondary"} flex items-center font-medium"
            on:click={() => {
            ...
            <p class="mx-[2vh] px-[2vh] py-[0.2vh] rounded-xl {dispatch.priority == 1 ? " bg-accent_dark_red" : "  bg-tertiary"} ">{dispatch.units.length} {$Locale.units}</p>
            <p class="ml-[3vh]">
              {#if CheckIfAttached(dispatch.units, $PLAYER.citizenid)}
