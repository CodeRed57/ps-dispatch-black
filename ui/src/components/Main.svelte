...
<div class="w-screen h-screen flex justify-end { $IS_RIGHT_MARGIN ? 'flex-row' : 'flex-row-reverse' } items-end">
  <div class="w-[25%] h-[97%]"
    ...
    {#each notifications.slice().reverse() as dispatch, index (dispatch.data.id)}
      <div class="w-full h-fit my-[0.5vh] rounded-xl font-medium {dispatch.data.priority == 1 ? " bg-priority_secondary" : " bg-secondary"}" transition:fly="{{ x: $IS_RIGHT_MARGIN ? 400 : -400 }}">
        <div class="flex items-center rounded-xl gap-[1vh] p-[1vh] text-[1.5vh] {dispatch.data.priority == 1 ? " bg-priority_primary" : " bg-primary"}">
          <p class="px-[2vh] py-[0.2vh] rounded-xl bg-tertiary">
            #{dispatch.data.id}
          </p>
          <p class="px-[2vh] py-[0.2vh] rounded-xl {dispatch.data.priority == 1 ? " bg-accent_red" : "bg-tertiary"}">
            {dispatch.data.code}
          </p>
          <p class="py-[0.2vh]">
            {dispatch.data.message}
          </p>
          <i class="{dispatch.data.icon} py-[0.2vh] rounded-xl ml-auto mr-[0.5vh] {dispatch.data.priority == 1 ? " text-accent_red" : "text-tertiary"}"></i>
        </div>
        <div class="flex">
          ...
          <div class="w-[30%] flex items-end justify-center mb-[1vh]">
            {#if index === 0}
              <p class="px-[1.5vh] py-[0.4vh] rounded-xl text-[1.3vh] {dispatch.data.priority == 1 ? " bg-priority_primary" : " bg-primary"}">
